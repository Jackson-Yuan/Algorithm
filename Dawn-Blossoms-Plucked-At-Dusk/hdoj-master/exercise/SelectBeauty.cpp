 ...
 * 由于策划师风之鱼（大师级水王）宣传到位，来应聘的MM很多，
 * 当然包括nit的蛋糕妹妹等呼声很高的美女，就连zjut的jqw都男扮女装来应聘（还好被安全顾问hdu_Bin-Laden认出，给轰走了），看来娱乐圈比acm还吸引人哪...
 * 面试那天，刚好来了m*n个MM，站成一个m*n的队列，副导演Fe(OH)2为每个MM打了分数，分数都是32位有符号整数。
 * 一开始我很纳闷：分数怎么还有负的？Fe(OH)2解释说，根据选拔规则，头发染成黄色、化妆太浓、穿的太少等等都要扣分数的，扣的多了就可能是负分了，
...
int main() {
    ...
    while (cin >> m >> n) {
        ...
        int max = realVal(res[0].val);
        int maxIndex = 0;
        for (int k = 1; k < res.size(); ++k) {
            if (realVal(res[k].val) > max) {
                max = realVal(res[k].val);
                maxIndex = k;
            }
        }

        cout << res[maxIndex].x << " " << res[maxIndex].y << " " << res[maxIndex].val << endl;
    }
